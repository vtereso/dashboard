...
func TestWebsocketResources(t *testing.T) {
	...
	for i := 1; i <= clients; i++ {
		...
		go func() {
			...
			for {
				...
				informerRecord.Handle(messageType)
			}
		}()
		wg.Add(1)
	}
	...
	for _, informerRecord := range recordMap {
		...
		// records without an update hook
		if updates == -1 {
...
func CDNamespaces(r *Resource, t *testing.T) {
	namespace := corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "newNamespace"}}
	t.Log("Creating namespace")
	_, err := r.K8sClient.CoreV1().Namespaces().Create(&namespace)
	if err != nil {
		t.Fatalf("Error creating namespace: %s: %s\n", namespace.Name, err.Error())
	}
	t.Log("Deleting namespace")
	err = r.K8sClient.CoreV1().Namespaces().Delete(namespace.Name, &metav1.DeleteOptions{})
	if err != nil {
