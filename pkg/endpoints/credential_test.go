...
func TestGETCredentials(t *testing.T) {
	...
	httpReq := dummyHTTPRequest("GET", fmt.Sprintf("%s/v1/namespaces/*/credentials", server.URL), nil)
	...
	if len(responseCredentialsList) != allSecrets {
		t.Fatalf("All namespace credentials were not returned: expected %v, actual %v", allSecrets, len(responseCredentialsList))
	}

	// Test getAllCredentials: each namespace
	for namespace, namespacedSecrets := range namespaceSecretMap {
		httpReq = dummyHTTPRequest("GET", fmt.Sprintf("%s/v1/namespaces/%s/credentials", server.URL, namespace), nil)
		response, err = http.DefaultClient.Do(httpReq)
		if err != nil {
			t.Fatalf("getAllCredentials response error: %v\n", err)
		}
	...
	for _, namespacedSecrets := range namespaceSecretMap {
		for _, secret := range namespacedSecrets {
			httpReq = dummyHTTPRequest("GET", fmt.Sprintf("%s/v1/namespaces/%s/credentials/%s", server.URL, secret.Namespace, secret.Name), nil)
			response, err = http.DefaultClient.Do(httpReq)
			if err != nil {
...
func TestPUTCredentials(t *testing.T) {
	...
	for _, credential := range credentials {
		...
		httpReq := dummyHTTPRequest("PUT", fmt.Sprintf("%s/v1/namespaces/%s/credentials/%s", server.URL, credential.Namespace, credential.Name), httpRequestBody)
...
func TestDELETECredentials(t *testing.T) {
	...
	for _, credential := range credentials {
		...
		postAndCheckCredential(server, credential, r, t)
		// Delete credential
		httpReq := dummyHTTPRequest("DELETE", fmt.Sprintf("%s/v1/namespaces/%s/credentials/%s", server.URL, credential.Namespace, credential.Name), nil)
		_, err := http.DefaultClient.Do(httpReq)
		if err != nil {
			t.Fatal(err)
		}
...
func postAndCheckCredential(server *httptest.Server, credential Credential, r *Resource, t *testing.T) {
	...
	httpRequestBody := bytes.NewReader(jsonMarshalledBytes)
	httpReq := dummyHTTPRequest("POST", fmt.Sprintf("%s/v1/namespaces/%s/credentials", server.URL, credential.Namespace), httpRequestBody)
	_, err = http.DefaultClient.Do(httpReq)
	if err != nil {
